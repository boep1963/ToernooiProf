...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  // Build nav items: base items + conditional admin items (Admin, Issues & wensen) + trailing items
  const navItems = useMemo(() => {
    const items = [...baseNavItems];
    if (isSuperAdmin) {
      items.push(adminNavItem);
      items.push(issuesNavItem);
    }
    items.push(...trailingNavItems);
    return items;
  }, [isSuperAdmin]);
  ...
  return (
    <>
      ...
      >
        ...
        {/* Navigation */}
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto" aria-label="Hoofdnavigatie">
          {navItems.map((item) => {
