...
export default function ToernooiStandPage({
  params,
}: {
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3 print:hidden">
        ...
        <div className="ml-auto flex gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 text-sm bg-orange-600 hover:bg-orange-700 disabled:bg-orange-400 text-white font-medium rounded-lg transition-colors shadow-sm"
          >
                ...
                {sortedStandings.map((entry, index) => (
                  ...
                  >
                      ...
                      {entry.matchesPlayed > 0 && entry.rank >= 1 && entry.rank <= 3 ? (
                        <span
                          className={`inline-flex items-center justify-center w-6 h-6 text-white text-xs rounded-full ${
                            entry.rank === 1 ? 'bg-orange-700' :
                            entry.rank === 2 ? 'bg-orange-600' :
                            'bg-orange-500'
                          }`}
                        >
                          {entry.rank}
                    ...
                    {sortByPercentage ? (
                      ...
                      <td className="text-right px-3 py-2.5 text-sm font-bold text-orange-600 dark:text-orange-400 tabular-nums">
                        {entry.punten}
                      </td>
                    )}
