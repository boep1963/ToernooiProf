          ...
          <div className="flex items-center justify-between mb-3">
            ...
            {groupedTargetOverview.length > 0 && (
              <button
                ...
                className="print:hidden inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-slate-600 dark:text-slate-300 border border-slate-300 dark:border-slate-600 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
              >
          ...
          <div className="space-y-3">
            ...
            {groupedTargetOverview.map(([pouleNr, items]) => (
              <div key={pouleNr} className="rounded-lg border border-slate-200 dark:border-slate-700 p-3">
                <div className="text-sm font-semibold mb-2">Poule {pouleNr} ({items.length} spelers)</div>
                <table className="w-full text-xs">
                  <thead className="text-slate-500 dark:text-slate-400">
                    <tr>
                      <th className="text-left pb-1 font-medium">Speler</th>
                      <th className="text-right pb-1 font-medium">Moy</th>
                      <th className="text-right pb-1 font-medium">Car</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                    {items.map((it) => (
                      <tr key={it.sp_nummer}>
                        <td className="py-1 text-slate-900 dark:text-white">{it.sp_naam}</td>
                        <td className="py-1 text-right text-slate-600 dark:text-slate-400">{formatDecimal(it.moy_start)}</td>
                        <td className="py-1 text-right text-slate-600 dark:text-slate-400">{it.car_start}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
