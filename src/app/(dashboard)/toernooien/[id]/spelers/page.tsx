...
export default function ToernooiSpelersPage({
  ...
  // Get members not yet in the toernooi and with moyenne for this discipline
  ...
  // Get the moyenne for the selected member for the toernooi's discipline
  ...
  const handleAddPlayer = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSuccess(`${naam} is succesvol toegevoegd aan het toernooi!`);
  ...
  const handleBulkAddPlayers = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setBulkMode(false);
        setSelectedMembers([]);
        setSuccess(`${data.count} speler(s) succesvol toegevoegd aan het toernooi!`);
  ...
  const handleRemovePlayer = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSuccess(`${naam} is succesvol verwijderd uit het toernooi!`);
        setTimeout(() => setSuccess(''), 4000);
  ...
  // Sort players based on toernooi's sorteren setting
        ...
        <div className="flex items-start justify-between mb-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="px-3 py-1.5 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none transition-colors"
            >
        ...
        <div className="mb-6 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            {availableMembers.length > 0 && (
              <button
                ...
                className="text-sm text-orange-600 dark:text-orange-400 hover:text-orange-800 dark:hover:text-orange-300 hover:underline font-medium transition-colors"
              >
          ...
          {availableMembers.length === 0 ? (
            <p className="text-slate-500 dark:text-slate-400 text-sm">
              Alle leden zijn al toegevoegd aan dit toernooi, of er zijn nog geen leden aangemaakt.
            </p>
          ) : bulkMode ? (
            <>
              <div className="mb-4">
                  ...
                  {availableMembers.map((m) => (
                    ...
                    >
                      <input
                        ...
                        className="w-4 h-4 text-orange-600 bg-white dark:bg-slate-700 border-slate-300 dark:border-slate-600 rounded focus:ring-2 focus:ring-orange-500"
                      />
      ...
      {!showAddDialog && (
        <div className="mb-4 flex items-center gap-3 print:hidden">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-orange-600 hover:bg-orange-700 disabled:bg-orange-400 text-white font-medium rounded-lg transition-colors shadow-sm"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              ...
              {sortedPlayers.length !== players.length && (
                <span className="ml-2 text-slate-400">
                  ({players.length} spelers gekoppeld aan toernooi)
                </span>
              )}
            </p>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl max-w-lg w-full p-6 border border-slate-200 dark:border-slate-700">
            ...
            <div className="mb-4 p-4 bg-red-50 dark:bg-red-900/30 rounded-lg border border-red-200 dark:border-red-800">
              ...
              <p className="mt-3 text-xs text-red-600 dark:text-red-400 font-medium">
                De speler kan wel opnieuw worden toegevoegd aan het toernooi.
              </p>
            </div>
