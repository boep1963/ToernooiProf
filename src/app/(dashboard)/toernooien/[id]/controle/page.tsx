...
export default function ToernooiControlePage({
  params,
}: {
  ...
  if (isLoading) {
    return (
      <div>
        <CompetitionSubNav compNr={compNr} compNaam="Toernooi" />
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-8">
          <LoadingSpinner size="lg" label="Validatie uitvoeren..." />
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      <CompetitionSubNav compNr={compNr} compNaam={report.tournament.comp_naam} periode={report.tournament.periode} />
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Validatie van {report.tournament.comp_naam}
            </p>
          </div>
          <button
            onClick={runValidation}
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-blue-800 dark:text-blue-300 leading-relaxed">
              Deze controle valideert de consistentie van uw toernooidata. Er worden checks uitgevoerd op:
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className={`bg-white dark:bg-slate-800 rounded-xl shadow-sm border ${(report.errors + report.warnings) === 0 ? 'border-orange-200 dark:border-orange-800' : 'border-slate-200 dark:border-slate-700'} p-5`}>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">
                Totaal Problemen
              </p>
              <p className={`text-2xl font-bold ${(report.errors + report.warnings) === 0 ? 'text-orange-600 dark:text-orange-400' : 'text-slate-900 dark:text-white'}`}>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-lg bg-orange-50 dark:bg-orange-900/30 flex items-center justify-center flex-shrink-0">
              <svg className="w-6 h-6 text-orange-600 dark:text-orange-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {report.totalIssues === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-slate-500 dark:text-slate-400">
              Er zijn geen problemen gevonden met de toernooidata.
            </p>
          </div>
        ) : (
