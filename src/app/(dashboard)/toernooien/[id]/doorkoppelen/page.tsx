...
export default function DoorkoppelenPage({
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (!compRes.ok) throw new Error('Toernooi niet gevonden');
  ...
  return (
    <div>
      <CompetitionSubNav compNr={compNr} compNaam={competition?.comp_naam || 'Toernooi'} periode={competition?.periode || 1} />
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-6">
        <div className="flex gap-3">
          ...
          <div className="text-sm text-blue-800 dark:text-blue-200">
            ...
            <p className="mb-3">
              <strong>Doorkoppelen past het moyenne van leden aan op basis van hun gespeelde moyenne in dit toernooi.</strong>
            </p>
            <p>
              Selecteer spelers en een periode. Het moyenne uit die periode wordt overgenomen als het nieuwe
              {' '}{DISCIPLINES[competition?.discipline || 1]} moyenne van de speler in het deelnemersbestand.
              Dit is handig bij het afsluiten van een toernooi, zodat het bijgewerkte moyenne automatisch
              als startwaarde wordt gebruikt voor het volgende toernooi.
            </p>
          </div>
        </div>
      </div>
              ...
              <thead className="bg-slate-50 dark:bg-slate-900">
                <tr>
                  ...
                  {[1, 2, 3, 4, 5].map((period) => (
                    <th
                      ...
                      className={`px-4 py-3 text-center text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider ${
                        selectedPeriod === period ? 'bg-orange-50 dark:bg-orange-900/20' : ''
                  ...
                  <th className={`px-4 py-3 text-center text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider ${
                    selectedPeriod === 6 ? 'bg-orange-50 dark:bg-orange-900/20' : ''
                  }`}>
                    Totaal
                  </th>
                </tr>
                ...
                {players.map((player) => (
                  <tr
                    ...
                    className={`hover:bg-slate-50 dark:hover:bg-slate-700/50 ${
                      selectedPlayers.has(player.spc_nummer) ? 'bg-orange-50 dark:bg-orange-900/10' : ''
                    }`}
                  >
                    ...
                    {[1, 2, 3, 4, 5].map((period) => (
                      <td
                        ...
                        className={`px-4 py-3 whitespace-nowrap text-sm text-center text-slate-700 dark:text-slate-300 ${
                          selectedPeriod === period ? 'bg-orange-50 dark:bg-orange-900/10 font-semibold' : ''
                        } ${competition && competition.periode < period ? 'opacity-50' : ''}`}
                      >
                    ...
                    <td className={`px-4 py-3 whitespace-nowrap text-sm text-center text-slate-700 dark:text-slate-300 ${
                      selectedPeriod === 6 ? 'bg-orange-50 dark:bg-orange-900/10 font-semibold' : ''
                    }`}>
