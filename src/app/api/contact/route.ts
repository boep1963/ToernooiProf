...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { onderwerp, bericht } = body;

    if (!onderwerp || !bericht) {
      return NextResponse.json({ error: 'Onderwerp en bericht zijn verplicht.' }, { status: 400 });
    }

    if (!VALID_SUBJECTS.includes(onderwerp)) {
      return NextResponse.json({ error: 'Ongeldig onderwerp geselecteerd.' }, { status: 400 });
    }
    ...
    // Fetch organization details to get email
    ...
    console.log(`[CONTACT] Subject: ${onderwerp}`);
