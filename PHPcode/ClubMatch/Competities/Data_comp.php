...
try {
	...
	while ($resultaat = mysqli_fetch_array($res, MYSQLI_BOTH)) {
		...
		$P_sys = $resultaat['punten_sys'];
		if (substr($P_sys, 0, 1) == 1) {
			$Punten_sys = "Systeem Winst = 2 punten, Remise = 1 punt en Verlies = 0 punten; ";
			if (substr($P_sys, 1, 1) == 0) {
				$Punten_sys = $Punten_sys . "geen extra punten.";
			} else {
				//$W = 1; standaard
				$Punten_sys = $Punten_sys . "met extra punt bij winst, ";
				$R = substr($P_sys, 3, 1);
				if ($R == 1) {
					$Punten_sys = $Punten_sys . "extra punt bij remise ";
				} else {
					$Punten_sys = $Punten_sys . "geen extra punt bij remise ";
				}
				$V = substr($P_sys, 4, 1);
				if ($V == 1) {
					$Punten_sys = $Punten_sys . "en extra punt bij verlies.";
				} else {
					$Punten_sys = $Punten_sys . "en geen extra punt bij verlies.";
				}
			}
		}
		...
		if ($Moy_form == 1) {
			$Moy_formule = "Aantal te maken car = moy_speler x 15";
		}
		if ($Moy_form == 2) {
		...
		$Vast_brt  = $resultaat['vast_beurten'];
		if ($Vast_brt == 0) {
			$Vast_beurten = "Geen vast aantal beurten";
		} else {
			$Vast_beurten = $Vast_brt;
		}
	}
...
<head>
	<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
	<title>Competitie-gegevens</title>
	<meta name="Keywords" content="Biljarten, Competitie, Hans Eekels" />
...
<body>
	<form name="nieuw" method="post" action="Competitie_beheer.php">
		<table width="700" border="0">
			...
			<tr>
				<td height="40" align="left" valign="middle" bgcolor="#009900" class="grootwit">&nbsp;Vast aantal beurten</td>
				<td align="left" bgcolor="#009900">&nbsp;<?php print("$Vast_beurten"); ?></td>
			</tr>
			<tr>
