<?php
//Startpagina Dashboard ClubMatch Hans Eekels, versie 01-01-2026
//reminder send is kleur rood
require_once('../../data/connectie_clubmatch.php');
/*
var_dump($_POST);
array(1) { ["inactief"]=> string(4) "Kies" }	of nummer of email
*/
$Accounts = array();
...
//$Datum_gebruiker = new DateTime("2025-01-01");
//$verschil = $Datum_nu->diff($Datum_gebruiker);
...
try {
	...
	{	
		$sql = "SELECT * FROM bj_organisaties ORDER BY org_nummer";
	}
	if ($Keuze == 2)	//sorteren op email
	{	
		$sql = "SELECT * FROM bj_organisaties ORDER BY org_wl_email";
	}
	if ($Keuze == 3)	//sorteren op dagen inactief
	{	
		$sql = "SELECT * FROM bj_organisaties ORDER BY date_inlog";
	}
	...
	if (!$res) {
		...
		if (mysqli_num_rows($res) == 0) {
			...
			while ($resultaat = mysqli_fetch_array($res, MYSQLI_BOTH)) {
				$teller++;
				$Accounts[$teller]['org_nr'] = $resultaat['org_nummer'];
				$Accounts[$teller]['code'] = $resultaat['org_code'];
				$Accounts[$teller]['org_naam'] = $resultaat['org_naam'];
				$Accounts[$teller]['wl_naam'] = $resultaat['org_wl_naam'];
				$Accounts[$teller]['email'] = $resultaat['org_wl_email'];
				...
				$Accounts[$teller]['aantal_comp'] = 0;	//hierna vullen
		...
		if ($Aantal_accounts > 0)
		{
			...
			{
				$Org_nr = $Accounts[$a]['org_nr'];
				$sql = "SELECT * FROM bj_competities WHERE org_nummer = '$Org_nr'";
				...
				$Accounts[$a]['aantal_comp'] = mysqli_num_rows($res);
			}
		}
	...
	<div id="printableArea">
  	<table width="960" border="1">
    	<tr>
            ...
            <strong><?php print("Accounts ClubMatch Online [$Datum_huidig] gesorteerd op $Naam_hulp")?></strong>
            </td>
        <tr>
        <tr>
            ...
            <td width="60" align="center" bgcolor="#FFFFFF" class="zwart"><strong># comp</strong></td>
		...
		if ($Aantal_accounts > 0)
		{
			...
			{
				$Org_nr = $Accounts[$a]['org_nr'];
				$Code = $Accounts[$a]['code'];
				$Org_naam = $Accounts[$a]['org_naam'];
				...
				$Aantal_comp = $Accounts[$a]['aantal_comp'];
				$Inactief = $Accounts[$a]['inactief'];
				$Reminder_send = $Accounts[$a]['reminder_send'];
				?>
				<tr>
					<td height="30" align="center" bgcolor="#FFFFFF" class="zwart"><?php print("$Org_nr"); ?></td>
					...
					{
						?>
                        <td align="center" bgcolor="#FF0000" class="wit"><?php print("$Aantal_comp"); ?></td>
                    	<?php
					}
					else
					{
						?>
                        <td align="center" bgcolor="#FFFFFF" class="zwart"><?php print("$Aantal_comp"); ?></td>
                    	<?php
					}
                    if ($Reminder_send == 1)
					{
                    	?>
						<td align="center" bgcolor="#FF0000" class="wit"><?php print("$Inactief"); ?></td>
                        <?php
                    }
                    else
                    {
                    	?>
						<td align="center" bgcolor="#FFFFFF" class="zwart"><?php print("$Inactief"); ?></td>
                        <?php
                    }
                    ?>
				</tr>
