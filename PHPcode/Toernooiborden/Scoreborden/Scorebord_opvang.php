...
["toernooi_nr"]=> string(1) "1" 				//toernooi-nummer
["u_code"]=> string(3) "2_1" 					//uitslagcode
["poule_nr"]=> string(1) "1" 				//poule nummer
...
if (isset($_POST['switch'])) {
	...
	//kan alleen bij start en geen record in tp_uitslag_hulp (is al getest)
	try {
		...
		$sql = "SELECT * FROM tp_uitslagen 
			WHERE sp_partcode = '$U_code' AND gebruiker_nr = '$Gebruiker_nr' AND t_nummer = '$Toernooi_nr' AND sp_poule = '$Poule_nr' AND t_ronde = '$Huidige_ronde'";
		...
		while ($resultaat = mysqli_fetch_array($res, MYSQLI_BOTH)) {
			...
			$Volg1_oud = $resultaat['sp_volgnummer_1'];
			$Car1_oud = $resultaat['sp1_car_tem'];

			$Nr2_oud = $resultaat['sp_nummer_2'];
			$Volg2_oud = $resultaat['sp_volgnummer_2'];
			$Car2_oud = $resultaat['sp2_car_tem'];
		}

		//omwisselen met update in tp_uitslagen
		$sql = "UPDATE tp_uitslagen 
			SET sp_nummer_1 = '$Nr2_oud', sp_volgnummer_1 = '$Volg2_oud', sp1_car_tem = '$Car2_oud', sp_nummer_2 = '$Nr1_oud', sp_volgnummer_2 = '$Volg1_oud', sp2_car_tem = '$Car1_oud'
	...
	*/
	...
	try {
		...
		if (mysqli_num_rows($res) == 0) {
			...
			$sql = "INSERT INTO tp_uitslag_hulp (gebruiker_nr, t_nummer, t_ronde, poule_nr, uitslag_code, car_A_tem, car_A_gem, hs_A, brt, car_B_tem, car_B_gem, hs_B, turn, alert) 
				VALUES ('$Gebruiker_nr', '$Toernooi_nr', '$Huidige_ronde', '$Poule_nr', '$U_code', '$Car_A_tem', '$Car_A_gem', '$scoreA', '$Brt', '$Car_B_tem', '$Car_B_gem', '0', '1', '$Alert')";
			...
			if ($Turn == 1) {
				...
				$sql = "INSERT INTO tp_uitslag_hulp (gebruiker_nr, t_nummer, t_ronde, poule_nr, uitslag_code, car_A_tem, car_A_gem, hs_A, brt, car_B_tem, car_B_gem, hs_B, turn, alert) 
					VALUES ('$Gebruiker_nr', '$Toernooi_nr', '$Huidige_ronde', '$Poule_nr', '$U_code', '$Car_A_tem', '$Car_A_gem', '$Hs_A', '$Brt', '$Car_B_tem', '$Car_B_gem', '$Hs_B', '1', '$Alert')";
			...
			if ($Turn == 2) {
				...
				$Brt = $Beurten;
				$sql = "UPDATE tp_uitslag_hulp SET car_B_gem = '$Car_B_gem', hs_B = '$Hs_B', turn = '2', alert = '$Alert' 
					WHERE gebruiker_nr = '$Gebruiker_nr' AND t_nummer = '$Toernooi_nr' AND t_ronde = '$Huidige_ronde' AND  poule_nr = '$Poule_nr' AND uitslag_code = '$U_code' AND brt = '$Beurten'";
	...
	if ($Turn == 2)		//is situatie op scherm
	{
		...
		*/
		...
		try {
			...
			$sql = "SELECT brt FROM tp_uitslag_hulp 
			...
			if ($Beurten > 1) {
				...
				$sql = "UPDATE tp_uitslag_hulp SET car_B_gem = '$Car_B_gem', hs_B = '$Hs_B', turn = '$Turn' 
				WHERE gebruiker_nr = '$Gebruiker_nr' AND t_nummer = '$Toernooi_nr' AND t_ronde = '$Huidige_ronde' AND poule_nr = '$Poule_nr' AND uitslag_code = '$U_code' AND brt = '$Beurten'";
...
if ($Verwijzing == 3) {
	//Cancel, stoppen en terug naar kies tafel
	try {
		$dbh = mysqli_connect($dbhostname, $dbusername, $dbpassword, $dbnaam);
		if (!$dbh) {
			throw new Exception(mysqli_connect_error());
