...
//Toon geen partijen die nu bezig zijn (op deze tafel bezig is status=1 of 2 in tp_tafel, of op een andere tafel bezig is, staan in tp_uitslag_hulp
//refresh aangepast
...
array(3) { ["toernooi_nr"]=> string(1) "1" ["user_code"]=> string(10) "1024_AHS@#" ["tafel_04"]=> string(0) "" }
*/
...
try {
  ...
  $sql = "SELECT * FROM tp_tafel 
	WHERE gebruiker_nr = '$Gebruiker_nr' AND t_nummer = '$Toernooi_nr' AND t_ronde = '$Huidige_ronde' AND tafel_nr = '$Tafel_nr' AND status > '0'";
  ...
  if ($bTonen == TRUE) {
    $sql = "SELECT * FROM tp_uitslagen 
		WHERE gebruiker_nr = '$Gebruiker_nr' AND t_nummer = '$Toernooi_nr' AND t_ronde = '$Huidige_ronde' AND gespeeld > '7' AND (tafel_nr = '0' OR tafel_nr = '$Tafel_nr') ORDER BY uitslag_id DESC";
    ...
    if (mysqli_num_rows($res) > 0) {
      ...
      while ($resultaat = mysqli_fetch_array($res, MYSQLI_BOTH)) {
        ...
        $Partijen_hulp[$teller]['naam_A'] = fun_spelersnaam($Gebruiker_nr, $Toernooi_nr, $SpA, $Path);
        $SpB = $resultaat['sp_nummer_2'];
        $Partijen_hulp[$teller]['naam_B'] = fun_spelersnaam($Gebruiker_nr, $Toernooi_nr, $SpB, $Path);
        $Partijen_hulp[$teller]['uitslag_code'] = $resultaat['sp_partcode'];
        $Partijen_hulp[$teller]['t_ronde'] = $resultaat['t_ronde'];
        $Partijen_hulp[$teller]['poule_nr'] = $resultaat['sp_poule'];
      }

      $Aantal_partijen_hulp = $teller;

      //nu check op partijen die al gestart zijn op andere tafels
      //dat kunnen zowel tablet-tafels als muis-tafels zijn, dus dan is er een record in tp_tafels met deze u-code
      //of een record in tp_uitslag_hulp
      $teller = 0;
      for ($a = 1; $a < $Aantal_partijen_hulp + 1; $a++) {
        $U_code_hulp = $Partijen_hulp[$a]['uitslag_code'];
        $Poule_nr_hulp = $Partijen_hulp[$a]['poule_nr'];
        $sql = "SELECT * FROM tp_tafel 
				WHERE gebruiker_nr = '$Gebruiker_nr' AND t_nummer = '$Toernooi_nr' AND t_ronde = '$Huidige_ronde' AND uitslag_code = '$U_code_hulp' AND poule_nr = '$Poule_nr_hulp'";
        ...
        if (mysqli_num_rows($res) == 0) {
          ...
          if (mysqli_num_rows($res) == 0) {
            ...
            $Partijen[$teller]['poule_nr'] = $Partijen_hulp[$a]['poule_nr'];
          }
        }
      }
    ...
    <table style="width:100%;" border="0">
      <tr>
        <td colspan="4" align="center" bgcolor="#003300">
          <h2><?php print("$Toernooi_naam Tafel nr: $Tafel_nr"); ?></h2>
        </td>
      </tr>
      <tr>
      ...
      ?>
        ...
        for ($a = 1; $a < $Aantal_partijen + 1; $a++) {
          ...
          $Poule_nr_hulp = $Partijen[$a]['poule_nr'];
        ?>
          <tr>
            <td align="center" valign="middle">
              <input type="submit" class="wissel-button" name="u_code" style="background-color:#FF3; color:#FF3;"
                value="<?php print("$U_code"); ?>" onMouseOver="mouseInBut(event)" onMouseOut="mouseOutBut(event)">
              <input type="hidden" name="poule_nr" value="<?php print("$Poule_nr_hulp"); ?>">
            </td>
            <td align="center" valign="middle">
