...
//check op bestaand record (alleen mogelijk na gebruik/misbruik history goback knop door gebruiker
...
for ($a = 1; $a < $Aantal_poules + 1; $a++) {
	$Aantal_spelers = fun_aantalspelersinpoule($Gebruiker_nr, $Toernooi_nr, $Huidige_ronde, $a, $Path);
	if ($Aantal_spelers < 2) {
...
?>
	...
	<head>
		<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
		<title>Fout in poule-indeling</title>
		<meta name="Keywords" content="Biljarten, Toernooi, Hans Eekels" />
		<meta name="Description" content="Toernooiprogramma" />
		<link rel="shortcut icon" href="Figuren/eekels.ico" type="image/x-icon" />
		<link href="PHP/StijlCentreren.css" rel="stylesheet" type="text/css" />
	...
	<body>
		<table width="500" border="0">
			...
			<tr>
				<td height="60" colspan="2" align="center" valign="middle" bgcolor="#003300">
					<form name="partijen" method="post" action="Spelers_Beheer.php">
						<input type="submit" class="submit-button" name="Beheer" value="Terug naar start" style="width:200px; height:40px; background-color:#0C0; color:#FFF; font-size:16px;"
							title="Naar start" onMouseOver="mouseInBut(event)" onMouseOut="mouseOutBut(event)">
...
try {
	...
	for ($Poule_teller = 1; $Poule_teller < $Aantal_poules + 1; $Poule_teller++) {
		$Aantal_spelers = fun_aantalspelersinpoule($Gebruiker_nr, $Toernooi_nr, $Huidige_ronde, $Poule_teller, $Path);

		//haal spelers in poule op
		$sql = "SELECT * FROM tp_poules WHERE gebruiker_nr = '$Gebruiker_nr' AND t_nummer = '$Toernooi_nr' AND poule_nr = '$Poule_teller' AND ronde_nr = '$Huidige_ronde'";
		...
		for ($a = 1; $a < $Aantal_spelers + 1; $a++) {
			...
			WHERE gebruiker_nr = '$Gebruiker_nr' AND t_nummer = '$Toernooi_nr' AND sp_nummer = '$Sp_nummer_hulp' AND poule_nr = '$Poule_teller' AND ronde_nr = '$Huidige_ronde'";
		...
		for ($Rn = 1; $Rn < $Aantal_ronden + 1; $Rn++) {
			for ($Kn = 1; $Kn < $Aantal_koppels + 1; $Kn++) {
				...
				WHERE gebruiker_nr = '$Gebruiker_nr' AND t_nummer = '$Toernooi_nr' AND sp_poule = '$Poule_teller' AND t_ronde = '$Huidige_ronde' AND sp_partcode = '$P_code' ";
				$res = mysqli_query($dbh, $sql);
				if (!$res) {
					throw new Exception(mysqli_error($dbh));
				}
				...
				if (mysqli_num_rows($res) == 0) {
					...
					VALUES 
					('$Gebruiker_nr', '$Toernooi_nr', '$sp1_nr', '$sp1_vnr', '$sp2_nr', '$sp2_vnr', '$Poule_teller', '$Huidige_ronde', '$Rn', '$Kn', '$P_code', 
	...
	//data
	$sql = "UPDATE tp_data SET t_gestart = '1', t_ronde = '$Huidige_ronde' WHERE gebruiker_nr = '$Gebruiker_nr' AND t_nummer = '$Toernooi_nr'";
...
<body>
	<table width="500" border="0">
		<tr>
			<td width="150" height="77" align="center" valign="middle" bgcolor="#003300"><img src="Figuren/Logo_standaard.jpg" width="150" height="75" alt="Logo" /></td>
			<td width="340" align="right" valign="middle" bgcolor="#003300">
				<h1><img src="Figuren/Startvlag.jpg" width="76" height="75" alt="gestart"></h1>
			</td>
		</tr>
		<tr>
			<td height="100" colspan="2" align="center">
				<div style="margin-left:5px; margin-right:5px; margin-bottom:5px; margin-top:5px; font-size:16px; font-weight:bold; background-color:#F00; color:#FFF;">
					Toernooi <?php print("$Toernooi_naam"); ?> is gestart !<br><br>
					U komt in het Toernooi Beheersscherm waar u alle acties kunt kiezen die u ondersteunen bij de organisatie van uw toernooi.<br><br>
				</div>
			</td>
		</tr>
		<tr>
