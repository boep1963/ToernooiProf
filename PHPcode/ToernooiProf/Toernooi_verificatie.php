...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        $sql = "SELECT * FROM tp_gebruikers WHERE tp_wl_email = '$email' AND return_code = '$return_code'";
        ...
        if (mysqli_num_rows($res) > 0) // Fix the condition to check if the code exists
        {
            while ($resultaat = mysqli_fetch_array($res, MYSQLI_BOTH)) {
                ...
                $Org_code = $resultaat['gebruiker_code'];
            }
            ...
            if (($current_time - $time_start) <= 900) {
                ...
                $sql = "UPDATE tp_gebruikers SET code_ontvangen = '1' WHERE tp_wl_email = '$email' AND return_code = '$return_code'";
                $res = mysqli_query($dbh, $sql);
                if (!$res) {
                    throw new Exception(mysqli_error($dbh));
                }
    ...
    if ($bGeslaagd == TRUE) {
        ...
        //email zenden
        $msg = "Nieuw account ($Org_code) aangemaakt voor ToernooiProf Online door $email";
...
?>
        ...
        <body>
            <form name="geslaagd" method="post" action="Toernooi_inloggen.php">
                <table width="800" border="0">
                    ...
                    <tr>
                        <td height="85" align="left" valign="middle" bgcolor="#009900"><img src="Figuren/Logo_standaard.jpg" width="150" height="75" alt="Logo" /></td>
    ...
    <body>
        <form name="verificatie" method="post" action="Toernooi_verificatie.php">
            <table width="800" border="0">
                <tr>
