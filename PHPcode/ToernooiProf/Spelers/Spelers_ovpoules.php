...
$Aantal_poules = fun_aantalpoules($Gebruiker_nr, $Toernooi_nr, 1, $Path);
...
try {
	...
	if (mysqli_num_rows($res) == 0) {
		...
		while ($resultaat = mysqli_fetch_array($res, MYSQLI_BOTH)) {
			$teller++;
			$Spelers[$teller]['poule_nummer'] = 0;		//hierna toevoegen
			$Spelers[$teller]['sp_naam'] = $resultaat['sp_naam'];
			$Moy = $resultaat['sp_startmoy'];
			$Car = $resultaat['sp_startcar'];
			$Sp_nr = $resultaat['sp_nummer'];
			$Spelers[$teller]['sp_moy'] = $Moy;
			$Spelers[$teller]['sp_car'] = $Car;
	...
	if ($Aantal_spelers > 0) {
		for ($a = 1; $a < $Aantal_spelers + 1; $a++) {
			...
			$sql = "SELECT poule_nr FROM tp_poules WHERE gebruiker_nr = '$Gebruiker_nr' AND t_nummer = '$Toernooi_nr' AND sp_nummer = '$Nr_sp' AND ronde_nr = '1'";
			...
			while ($resultaat = mysqli_fetch_array($res, MYSQLI_BOTH)) {
				$Spelers[$a]['poule_nummer'] = $resultaat['poule_nr'];
			}
		}	//end for per speler
	}	//end if #spelers > 0
...
<body>
	<table width="600" border="0">
		<tr>
			...
			<td width="376" align="center" valign="middle" bgcolor="#006600" class="grootwit">
				...
				<div class="scroll">
					<div id="printableArea">
						<table width="570" border="1">
							...
							<tr>
								<td colspan="3" align="center" class="grootzwart"><strong>Overzicht Poules met Spelers</strong></td>
							</tr>
							<?php
							if ($Aantal_spelers > 0) {
								$teller = 0;
	...
	<form name="cancel" method="post" action="../Spelers_Beheer.php">
		<table width="600">
			<tr>
				...
				<td width="170" align="center" bgcolor="#006600">
