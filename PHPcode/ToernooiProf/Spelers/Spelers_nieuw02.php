...
var_dump($_POST);
array(6) { 
["naam"]=> string(9) "Henk Test" 
["Moyenne"]=> string(5) "0.350" 
["Car"]=> string(2) "14" 
["poule_nr"]=> string(1) "1" 
["t_nummer"]=> string(1) "3" 
...
//NB: elke speler heeft een uniek spelersnummer en per ronde een uniek nummer in de poule (die worden aangemaakt bij aanmaak rooster)
try {
	...
	//uniek nummer speler bepalen
	$sql = "SELECT sp_nummer FROM tp_spelers WHERE gebruiker_nr = '$Gebruiker_nr' AND t_nummer = '$Toernooi_nr' ORDER BY sp_nummer DESC limit 1";
	...
	if (mysqli_num_rows($res) == 0) {
		...
		while ($resultaat = mysqli_fetch_array($res, MYSQLI_BOTH)) {
			$Sp_nr = $resultaat['sp_nummer'] + 1;
		}
	}

	//check
	$sql = "SELECT * FROM tp_spelers WHERE gebruiker_nr = '$Gebruiker_nr' AND t_nummer = '$Toernooi_nr' AND sp_nummer = '$Sp_nr'";
	$res = mysqli_query($dbh, $sql);
	if (!$res) {
		throw new Exception(mysqli_error($dbh));
	}
	...
	if (mysqli_num_rows($res) == 0) {
		//opslaan in tp_spelers
		$sql = "INSERT INTO tp_spelers (gebruiker_nr, t_nummer, sp_nummer, sp_naam, sp_startmoy, sp_startcar) 
		VALUES ('$Gebruiker_nr', '$Toernooi_nr', '$Sp_nr', '$Naam', '$Moyenne', '$Car')";
		...
		VALUES ('$Gebruiker_nr', '$Toernooi_nr', '$Sp_nr', '$Moyenne', '$Car', '0', '$Poule_nr', '$Ronde_nr')";
...
//Melding
$error_message = "Speler opgeslagen.<br>U keert terug naar de pagina spelers invoeren";
?>
<!DOCTYPE html>
<html>
	...
	<table width="500" border="0">
		...
		<tr>
			<td colspan="2" height="60" align="center" valign="middle" bgcolor="#006600">
				<form name="akkoord" method="post" action="Spelers_nieuw01.php">
