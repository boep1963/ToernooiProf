...
$Copy = Date("Y");
/*
var_dump($_POST) geeft: check @@@
array(5) { 
["Moyenne"]=> string(5) "0.900" 
["Car"]
["poule_nr"]=> string(1) "2" 
...
if (!isset($_POST['poule_nr'])) {
	...
	$Poule_nr = $_POST['poule_nr'];	//kan 0 zijn
}

if (!isset($_POST['Moyenne'])) {
	$bAkkoord = FALSE;
} else {
	$Sp_moy = $_POST['moyenne_nieuw'];
	if ($Sp_moy < 0.250) {
		$Sp_moy = 0.250;
	}
}
...
if (!isset($_POST['Car'])) {
	...
	$Sp_car = $_POST['Car'];
	if (filter_var($Sp_car, FILTER_VALIDATE_INT) == FALSE) {
		$bAkkoord = FALSE;
	}
...
$Naam = fun_spelersnaam($Gebruiker_nr, $Toernooi_nr, $Sp_nummer, $Path);

//delete in poules als $Poule_nr = 0, anders updaten met $Poule_nr
try {
	...
	if ($Poule_nr == 0) {
		//delete speler uit poules ronde nieuw
		$sql = "DELETE FROM tp_poules 
			WHERE gebruiker_nr = '$Gebruiker_nr' AND t_nummer = '$Toernooi_nr' AND sp_nummer = '$Speler_nr' AND ronde_nr = '$Nieuwe_ronde'";
		...
		//update poule van speler
		$sql = "UPDATE tp_poules SET sp_moy = '$Sp_moy', sp_car = '$Sp_car', poule_nr = '$Poule_nr'
			WHERE gebruiker_nr = '$Gebruiker_nr' AND t_nummer = '$Toernooi_nr' AND sp_nummer = '$Speler_nr' AND ronde_nr = '$Nieuwe_ronde'";
...
//Melding
if ($Poule_nr == 0) {
	$error_message = "Speler $Naam is niet meer doorgekoppeld naar ronde $Nieuwe_ronde!<br>U keert terug naar de pagina ronde aanmaken.";
} else {
	$error_message = "Speler $Naam is in ronde $Nieuwe_ronde geplaatst in poulenummer $Poule_nr !<br>U keert terug naar de pagina ronde aanmaken.";
}
?>
<!DOCTYPE html>
<html>
...
<body>
	<table width="500" border="0">
		<tr>
			<td width="150" height="77" align="center" valign="middle" bgcolor="#006600"><img src="<?php print("$Logo_naam"); ?>" width="150" height="75" alt="Logo" /></td>
